...
-   Assume that patients who die in a cycle incur neither the state
    costs or cost of treatment.
...
The ICER for the new treatment (T1) versus conventional management (T0)
is $13,702 per QALY gained.

**Question: The healthcare payer is willing to pay $20,000 per
additional QALY. Based on the ICER, should they fund the new
treatment?**

Yes. At $13,702/QALY, the new treatment costs less per additional QALY
than the payer’s $20,000 willingness-to-pay threshold, so it would be
considered cost-effective and worth funding under this criterion.

**Question: Are the ICER values obtained from the Excel and the R script
identical? If not, why is that?  
(Hint: what does method=end in the run_model function mean? Search the
`heemod` package documentation.)**

No, the Excel gives an ICER of about $13,821/QALY, while the R script
gives about $13,702/QALY. The difference comes entirely from a modeling
assumption embedded in method = “end”. The R script calculates each
